...
export default function Home() {
  ...
  const handleApiKeySubmit = async () => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:5000/store-apikey', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ apikey, github_token: githubToken }),
      });

      if (res.status === 200) {
        onClose(); // Close the modal if the API key is stored successfully
      } else {
      ...
      <div className="relative min-h-screen">
        ...
        <div className="">
          <div className="shadow-md rounded-lg p-8 w-full">
            ...
            {response && (
              <Clipboard.WithIconText className="justify-end fixed top-10" valueToCopy={response} />
            )}
